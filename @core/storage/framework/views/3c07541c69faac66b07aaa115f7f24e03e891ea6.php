...
<?php endif; ?>
                        ...
                        <form action="<?php echo e(route('frontend.donations.log.store')); ?>" method="post" enctype="multipart/form-data" class="donation-form-wrapper">
                         <?php echo csrf_field(); ?>
                            ...
                            <div class="tab_section mb-4">
                                <nav>
                                    <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                        ...
                                        <a class="nav-link active once_tab" id="nav-home-tab" data-toggle="tab" href="#nav-home" role="tab" aria-controls="nav-home" aria-selected="true"><?php echo e(__('Once')); ?></a>
                                        <a class="nav-link ml-2 monthly_tab" id="nav-profile-tab" data-toggle="tab" href="#nav-profile" role="tab" aria-controls="nav-profile" aria-selected="false"><?php echo e(__('Monthly')); ?></a>
                                        <?php endif; ?>
                                    </div>
                                </nav>
                           ...
                           <div class="fields_switcher_paren d-flex justify-content-between">
                               <div class="form-grou my-3">
                                   <label for="navbar_button" class="text-dark"><?php echo e(__('Show/Hide Individual Fields')); ?></label>
                                   <label class="switch">
                                       <input type="checkbox" name="" data-status="ok" class="individual_fields_status_switcher"
                                              <?php if(!empty($donation->individual_fields_status)): ?> checked <?php endif; ?> >
                                       <span class="slider"></span>
                                   </label>
                               </div>

                               <div class="form-grou my-3">
                                   <label for="navbar_button" class="text-dark"><?php echo e(__('Show/Hide Company Fields')); ?></label>
                                   <label class="switch">
                                       <input type="checkbox" name="" data-status="" class="company_fields_status_switcher"
                                              <?php if(!empty($donation->company_fields_status)): ?> checked <?php endif; ?> >
                                       <span class="slider"></span>
                                   </label>
                               </div>
                           </div>
                      ...
                      <div class="all_fields_main_container">
                            ...
                            <?php if(!empty(get_static_option('donation_form_builder_data_show_hide'))): ?>
                             <?php echo render_form_field_for_frontend(get_static_option('donation_page_form_fields')); ?>

                            <?php endif; ?>

                            <?php
                                $individual_fields = ['address','country','nif','cc','birthday'];
                                $company_fields = ['nipc','cae','address','country'];
                            ?>

                           <div class="individual_fields_container mt-3">
                               <?php $__currentLoopData = $individual_fields; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ind): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                   <div class="form-group mt-2">
                                       <input type="text" name="individual_fields[]" placeholder="<?php echo e(\Illuminate\Support\Str::upper($ind)); ?>" value="" class="form-control">
                                   </div>
                               <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                           </div>

                            <div class="company_fields_container d-none">
                                <?php $__currentLoopData = $company_fields; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $comp): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="form-group mt-2">
                                    <input type="text" name="company_fields[]" placeholder="<?php echo e(\Illuminate\Support\Str::upper($comp)); ?>" value="" class="form-control">
                                </div>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </div>
                      </div>
    ...
    <script>
        (function ($) {
            ...
            $(document).ready(function () {
                ...
                //Individual and company works
                $('.individual_fields_status_switcher').attr('checked',true);
                $('.company_fields_status_switcher').attr('disabled',true);

                $(document).on('change','.individual_fields_status_switcher',function(){
                    if(this.checked){
                        $('.individual_fields_container').removeClass('d-none');
                        $('.company_fields_status_switcher').attr('disabled',true);
                    }else{
                        $('.individual_fields_container').addClass('d-none');
                        $('.company_fields_status_switcher').attr('disabled',false);
                    }
                });

                $(document).on('change','.company_fields_status_switcher',function(){

                    if(this.checked){
                        $('.company_fields_container').removeClass('d-none');
                        $('.individual_fields_status_switcher').attr('disabled',true);
                    }else{
                        $('.company_fields_container').addClass('d-none');
                        $('.individual_fields_status_switcher').attr('disabled',false);
                    }
                });
...
<?php echo $__env->make('frontend.frontend-page-master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH H:\xampp\htdocs\fundorex-form-builder-client\@core\resources\views/frontend/donations/donation-payment-separate-page.blade.php ENDPATH**/ ?>