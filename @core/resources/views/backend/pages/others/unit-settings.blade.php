...
@section('content')
                            ...
                            <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                ...
                                @foreach($all_units as $key => $data)
                                    <tr>
                                        ...
                                        <td>
                                            @can('country-delete')
                                                <x-delete-popover :url="route('admin.unit.delete',$data->id)"/>
                                            @endcan
                                            @can('country-edit')
                                                <a href="#"
                                                   data-toggle="modal"
                                                   ...
                                                   data-id="{{$data->id}}"
                                                   data-action="{{route('admin.unit.update')}}"
                                                   data-name="{{$data->name}}"
 ...
 @section('script')
     @include('backend.popup-modals.unit.add')
     @include('backend.popup-modals.unit.edit')

   <x-admin-press-datatable.js/>
   <x-btn.submit/>
   <x-btn.update/>
   <x-bulk-action-js :url="route('admin.unit.bulk.action')"/>
