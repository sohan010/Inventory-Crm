                ...
                <div class="card">
                    <div class="card-header">
                        <div class="d-flex justify-content-between">
                            <div class="left">
                                <h3 class="card-title">{{__('Al Trash Products')}}</h3>
                            </div>
                            <div class="right">
                               <a href="{{route('admin.product')}}" class="btn btn-info text-white">{{__('Got All Products')}}</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                            ...
                            <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                ...
                                @foreach($all_products as $data)
                                    <tr>
                                        ...
                                        <td>
                                            @can('product-edit')
                                                <a href="{{route('admin.product.trash.restore',$data->id)}}" data-toggle="tooltip" data-title="Restore" class="btn btn-outline-primary btn-sm mb-3 mr-1 product_edit_btn">
                                                    <i class="ti-reload"></i>
                                                </a>
                                            @endcan

                                            @can('product-delete')
                                                <x-delete-popover :url="route('admin.product.trash.delete',$data->id)"/>
                                            @endcan
                                        </td>
                                    </tr>
                                @endforeach
...
@section('script')
    ...
    <x-bulk-action-js url="{{route('admin.product.trash.bulk.action')}}"/>
@endsection
