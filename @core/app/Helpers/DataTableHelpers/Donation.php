...
HTML;
    ...
    public static function paymentInfoColumn($row){
        ...
        //Individual Fields
        $individual_fields_decoded = json_decode($row->individual_fields) ?? [];
        $output.= '<li class="mt-2"><strong class="mb-2 text-primary ">'.__("Individual Fields : ").'</strong>';
        foreach($individual_fields_decoded ?? [] as $key=> $field):
        ...
        //Company Fields
        $company_fields_decoded = json_decode($row->company_fields) ?? [];
        $output.= '<li class="mt-2"><strong class="mb-2 text-success">'.__("Company Fields : ").'</strong>';
        foreach($company_fields_decoded ?? [] as $key=> $field):
            $output.= '<div class="att">';
            $output.='<strong class="text-dark"> '.ucfirst($key).' : </strong>';
            $output.= '<span>' .$field.'</span>';
            $output.=  '</div></li>';
        ...
        $all_custom_fields = json_decode($fields) ?? [];
